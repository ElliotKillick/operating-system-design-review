...
For reference, the compiler injected `<DLL_NAME>!_delayLoadHelper2` helper function that calls `ntdll!LdrResolveDelayLoadedAPI` passes in a [`DELAY_IMPORT_DESCRIPTOR` entry](https://learn.microsoft.com/en-us/windows/win32/debug/pe-format#the-delay-load-directory-table) as its first argument.
...
## Read Windows Strings

Windows, including user-mode and kernel-mode, commonly passes around strings in `STRING` or `UNICODE_STRING` structures:

```
dt ntdll!_STRING @$argreg
```

```
dt ntdll!_UNICODE_STRING @$argreg
```

For instance, the `ntdll!LdrpLoadForwardedDll` function receives a `STRING` of the DLL name to load as its first arugment.
...
## Open WinDbg in Non-Invasive Mode

Create a shortcut with this target:

```cmd
C:\Windows\System32\conhost.exe --headless C:\Windows\System32\cmd.exe /c "set _NO_DEBUG_HEAP=1 && start "%LOCALAPPDATA%\Microsoft\WindowsApps\WinDbgX.exe" /pv /hd"
```

For some reason, setting the command-line options is not enough for me to disable heap debug allocations (which muddy up traces and can also really slow down trace execution times). So, we use some CMD ninja techniques to also pass in the relevant environment variable while still not having a CMD window momentarily shoved in our face before launching WinDbg, and all without using PowerShell (which is slower to start)!
