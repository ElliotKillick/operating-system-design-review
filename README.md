...
A reasonable person could expect that given Windows' focus on dynamic loading of [executable modules](data/windows/timeline-verification)—unlike the prevalent static linking of the time—it would have ensured a solid design for its core dynamic loading API. Especially since [Multics](https://en.wikipedia.org/wiki/Multics) had already pioneered the idea of dynamically linking libraries, [used memory-mapped files to share data](https://www.multicians.org/myths.html#nofile), and [supported dynamic loading](https://www.multicians.org/mspm/be-7-07.661209.loader.pdf) years before Microsoft was even founded. Indeed, the module handle implementation quirk in the library loader exemplifies an avoidable misstep—one that, like most Windows API oversights, exists as an outcome of Microsoft's expedient development style when creating their core technologies.
...
In Windows, the Process Environment Block (PEB) stores data that is global to the process. As opposed to linking with [data exports or symbols](code/windows/data-export/README.md) made available by libraries, the PEB is a bad model for several reasons:
...
Centralizing state is also bad because it encourages coarse-grained locking, thus increasing lock contention, as is true for the PEB with its single `PEB.FastPEBLock` for synchronizing access even to unrelated members in this data structure. Windows calls this critical section lock "fast" presumably because it has a spin count attached to it that optimizes for the typically short acquisition times of this lock. However, a heuristic that improves performance by busy waiting is not a better solution than implementing fine-grained locking, thus reducing waits to begin with. Common scenarios where acquiring the PEB lock is necessary includes [creating thread-local data](https://github.com/reactos/reactos/blob/54433319af31c2b49737469d36072153de375f4d/dll/win32/kernel32/client/thread.c#L1109) and accessing environment variables, whereas Unix systems typically use purpose-built locks or low-level atomic operations, often including MT-safe synchronization, in these cases.
...
[Accessing the PEB requires going through the TEB to reach it](https://github.com/reactos/reactos/blob/56c8c8ab4f16633398356087ae9923d7b71bb467/sdk/include/ndk/rtlfuncs.h#L1111), which adds a layer of indirection. Alternatively, one could call the NTDLL exported function of [`RtlGetCurrentPeb`](https://github.com/reactos/reactos/blob/56c8c8ab4f16633398356087ae9923d7b71bb467/dll/ntdll/rtl/libsupp.c#L63-L68) to make PEB access slower by also introducing a function call plus the slight overhead of dynamic linking.

On the microarchitectural level, accessing addresses with a non-zero segment base is [documented to increase load latency](https://devblogs.microsoft.com/oldnewthing/20220919-00/?p=107195#:~:text=segment%20base), at least on the x86-64 architecture. Registers outside of the general-purpose registers are also less likely to be cached in the CPU pipeline, leading to a slowdown.
...
- Complaint #26: Unix UGO > Windows ACLs
  ...
  - Like most everything, Unix got this right before MS-DOS or Windows even existed ([Microsoft would know](https://www.pcjs.org/software/pcx86/sys/unix/ibm/xenix/1.0/), pro tip: enter `fd /xenix.fd` as the boot program then run `ls -l`)
